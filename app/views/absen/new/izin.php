...
<div class="card card-primary card-outline" style="margin-top:10px; background:beige">
   <div class="card-body box-profile">
      ...
      <div class="tengah judul1" style="color:red">
         <b>Izin Mengajar guru mata pelajaran</b>
      </div>
      <div class="tengah huruf1 mb-1" style="font-size:22px">
         <b>
            ~~ Hari :
      ...
      foreach ($data['jadwal'] as $kelas) :
         if ($kelas->kelas != $currentKelas) :
            ...
            <table class="table tabeljadwal2 mb-4">
               <tr>
                  <th style="background-color:black; color:white; font-weight:bold">Jam
                     <br />
                     &nbsp;&nbsp;&nbsp;Kelas&nbsp;&nbsp;&nbsp;
                  </th>
                  <?php for ($i = 1; $i <= 10; $i++) : ?>
            ...
            <tr>
               <td style="background-color:red; color:white; font-weight:bold;font-size:20px"><?= $kelas->kelas; ?><?= $kelas->ruang; ?></td>
               <?php for ($i = 1; $i <= 10; $i++) : ?>
                  ...
                  <td style="background-color: <?= $warna ?>;">
                     <form action="#" method="post">
                        ...
                        <?php if ($cek_absen) { ?>
                           ...
                           <?php } ?>
                        <?php } else { ?>
                           <button type="button" class="button1" onclick="absen('<?= $kelas->kelas ?>','<?= $kelas->ruang ?>','<?= $i ?>', '<?= $tgl ?>')">~ Izin ~<br />Klik disini</button>
                        <?php } ?>
                     </form>
                  </td>
               <?php endfor; ?>
...
<script>
   function absen(kelas, ruang, jam, tanggal) {
      // Menampilkan SweetAlert2 dengan form input
      Swal.fire({
         //title: "Kelas : " + kelas + ruang + ", jam ke : " + jam,
         html: `
            <div style="margin-top:0px">
               <div style="margin-bottom:-20px">
                  <input type="text" id="mata_pelajaran" class="swal2-input" placeholder="Mata Pelajaran">
               </div>
               <div  style="margin-bottom:-20px">
                  <select id="jenis_izin" class="swal2-input" placeholder="jenis_izin">
               ...
               <div>
                  <input type="text" id="alasan_izin" class="swal2-input" placeholder="Alasan Izin" required>
               </div>
            Anda yakin ingin mengisi absen untuk kelas dan jam tersebut?
            </div>
        `,
         //icon: "warning",
         ...
         if (result.isConfirmed) {
            // Melakukan AJAX request dengan data input
            $.ajax({
               url: '<?= URLROOT ?>/absen/isi_absen1/' + kelas + '/' + ruang + '/' + jam + '/' + tanggal,
               type: 'POST', // Ganti dengan metode HTTP yang sesuai
               dataType: 'json',
               data: result.value, // Mengirim data input ke server
